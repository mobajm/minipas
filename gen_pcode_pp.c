...
# 1 "/usr/lib/gcc/i686-pc-linux-gnu/4.8.0/include/stdarg.h" 1 3 4
# 40 "/usr/lib/gcc/i686-pc-linux-gnu/4.8.0/include/stdarg.h" 3 4
...
pi_t* gen_exit(void)
{
  return make_pi(CO_EXIT, ((void *)0));
}

pcode_t* gen_while(while_t* while_n)
{
  pcode_t* pcode = ((void *)0);
  char *loop_lab = ((void *)0);


  sprintf(label, "%s%d", ("loop"), lab_cnt++);
  loop_lab = strdup(label);
  (pcode) = add_to_pcode((make_pi(CO_LABEL, vmval_from_label(loop_lab))), (pcode));


  (pcode) = add_to_pcode((make_pi(CO_PUSH, vmval_from_number(from_double(eval_expr((while_n->cond)))))), (pcode));
  sprintf(label, "%s%d", ("endloop"), lab_cnt++);
  (pcode) = add_to_pcode((make_pi(CO_JF, vmval_from_label(label))), (pcode));


  merge_pcodes(pcode, gen_statseq(while_n->body));


  (pcode) = add_to_pcode((make_pi(CO_JMP, vmval_from_label(loop_lab))), (pcode));
  (pcode) = add_to_pcode((make_pi(CO_LABEL, vmval_from_label(label))), (pcode));
...
pcode_t* gen_if(if_t* if_node)
{
  ...
  (pcode) = add_to_pcode((make_pi(CO_PUSH, vmval_from_number(from_double(eval_expr((if_node->branches->cond)))))), (pcode));

  if (elsifs) {

    sprintf(label, "%s%d", ("elseif"), lab_cnt++);
    (pcode) = add_to_pcode((make_pi(CO_JF, vmval_from_label(label))), (pcode));


    merge_pcodes(pcode, gen_statseq(if_node->branches->sseq));


    while (elsifs) {

      (pcode) = add_to_pcode((make_pi(CO_LABEL, vmval_from_label(label))), (pcode));
      (pcode) = add_to_pcode((make_pi(CO_PUSH, vmval_from_number(from_double(eval_expr((elsifs->cond)))))), (pcode));

      if (elsifs->next != ((void *)0))
        sprintf(label, "%s%d", ("elseif"), lab_cnt++);
      else
        sprintf(label, "%s%d", ("else"), lab_cnt++);

      (pcode) = add_to_pcode((make_pi(CO_JF, vmval_from_label(label))), (pcode));
      merge_pcodes(pcode, gen_statseq(elsifs->sseq));
      elsifs = elsifs->next;
    }
  } else {
    sprintf(label, "%s%d", ("else"), lab_cnt++);
  ...
  if (if_node->else_branch) {
    (pcode) = add_to_pcode((make_pi(CO_JF, vmval_from_label(label))), (pcode));
    merge_pcodes(pcode, gen_statseq(if_node->else_branch));
  }
  return pcode;
}
...
pcode_t* gen_assign(assign_t* ass_node)
{
  ...
  (pcode) = add_to_pcode((make_pi(CO_PUSH, vmval_from_number(from_double(eval_expr((ass_node->expr)))))), (pcode));
  (pcode) = add_to_pcode((make_pi(CO_STORE, vmval_from_ident(ass_node->desig))), (pcode));
...
{
  pcode_t* pcode_stat = ((void *)0);

  switch (s->k) {
    case EXIT: (pcode_stat) = add_to_pcode((gen_exit()), (pcode_stat)); break;
